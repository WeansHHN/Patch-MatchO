...
NSString* showAlertWithText(NSString* text) {
    UIAlertController *alert = [UIAlertController alertControllerWithTitle:NSSENCRYPT("Alert")
                                                                   message:text
                                                            preferredStyle:UIAlertControllerStyleAlert];
    
    [alert addAction:[UIAlertAction actionWithTitle:NSSENCRYPT("OK")
...
NSMutableData* add_hook_section(NSMutableData* macho)
{
    ...
    for (int i = 0; i < header->ncmds; i++) {
        ...
        if (lc->cmd == LC_SEGMENT_64)
        {
            ...
            {
                //printf("section[%d] = %s/%s offset=%x vm=%p:%p", j, sec[j].segname, sec[j].sectname, sec[j].offset, sec[j].addr, sec[j].size);
...
NSString* StaticInlineHookPatch(char* machoPath, uint64_t vaddr, char* patch)
{
    ...
    NSString* savePath = [NSString stringWithFormat:@"%@/Documents/WeansHHN(haininh.site)/%s", NSHomeDirectory(), machoPath];
